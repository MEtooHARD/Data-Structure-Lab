...
// search data
SearchResult* search(Node* root, double latitude, double longitude, int n) {
  SearchResult* results = (SearchResult*)malloc(sizeof(SearchResult) * n);
  for (int i = 0; i < n; i++) {
    results[i].distance = DBL_MAX;
    results[i].data.latitude = 0.0;
    results[i].data.longitude = 0.0;
    strcpy(results[i].data.name, "");
  }
  ...
  if (root->isLeaf) {
    ...
    for (int i = 0; i < root->count; i++) {
      SearchResult* childResults =
          search(root->children[i], latitude, longitude, n);
      for (int j = 0; j < n; j++) {
        for (int k = 0; k < n; k++) {
