...
class LinkList {
 public:
  ...
  ListNode<T> &operator[](int i) {
    ListNode<T> *j = head;
    int k;
    for (k = 0; k < i && j != NULL; k++) j = j->getNext();
  ...
  void BFS(WeightedGraphVertex<V, E> *v) {
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; (*current)[i] != NULL; i++) {
        WeightedGraphVertex<V, E> *neighbor =
            (*(*current)[i]).getData()->getAnotherEnd(current);
...
int main() {
  ...
  scanf("%d", &n);
  srand(n);
  for (j = 0; j < 26; j++) node->addFromTail(g->addVertex(j + 'A'));
  k = rand() % 100;
  for (j = 0; j < k; j++) {
    a = rand() % 26;
    b = rand() % 26;
