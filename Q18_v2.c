...
void Cmp_SwapData(Node* node, int i, int j, int axis) {
...
void Cmp_SwapNode(Node* node, int i, int j, int axis) {
...
Node* splitNode(Node* node) {
  ...
  for (int i = 0; i < node->count; i++)
    for (int j = 0; j < node->count; j++)
      if (node->isLeaf)
        Cmp_SwapData(node, i, j, axis);
      else
        Cmp_SwapNode(node, i, j, axis);
  /* sort data/node by axis */
  /* assign sorted data/node to new node */
  for (int i = node->count / 2; i < node->count; i++) /* HERE */
    if (node->isLeaf)
...
void SearchRecursive(Node* node, double latitude, double longitude,
                     SearchResult* arr, int n) {
  if (node->isLeaf) {
    for (int I = 0; I < node->count; I++) {
      double dis = distance(latitude, longitude, node->data[I].latitude,
                            node->data[I].longitude);

      for (int II = n - 1; II >= 0; II--) {
        if (dis < arr[II].distance) {
          if (II < n - 1) arr[II + 1] = arr[II];         // shift
          if (II == 0 || dis >= arr[II - 1].distance) {  // insert
            arr[II].distance = dis;
            arr[II].data = node->data[I];
            break;
          }
        } else
          break;
    ...
    for (int I = 0; I < node->count; I++)
      SearchRecursive(node->children[I], latitude, longitude, arr, n);
  }
}

SearchResult* search(Node* node, double latitude, double longitude, int n) {
  SearchResult* arr = (SearchResult*)malloc(sizeof(SearchResult) * (n + 1));
  for (int i = 0; i < n; i++) {
    arr[i].distance = DBL_MAX;
    arr[i].data.latitude = 0.0;
    arr[i].data.longitude = 0.0;
    strcpy(arr[i].data.name, "");
  }
  SearchRecursive(node, latitude, longitude, arr, n);
  return arr;
}
