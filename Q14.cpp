...
class MinMaxHeap {
 public:
  ...
  T getMax() const {
    ...
    if (heap.size() >= 2) return heap[heap.size() - 1];
 ...
 private:
  ...
  void check_min(int index) {
    ...
    for (int i = 1; i <= 2; ++i) {
      ...
      if (grandchild < heap.size() && heap[grandchild] < heap[min])
        min = grandchild;
  ...
  void check_max(int index) {
    ...
    for (int i = 1; i <= 2; ++i) {
      int grandchild = 2 * left + i;
      if (grandchild < heap.size() && heap[grandchild] > heap[max])
        max = grandchild;
      grandchild = 2 * right + i;
      if (grandchild < heap.size() && heap[grandchild] > heap[max])
        max = grandchild;
...
int main() {
  ...
  int j;
  srand(time(NULL));
  for (j = 0; j < 10; j++) mmHeap.insert(rand() % 100);
